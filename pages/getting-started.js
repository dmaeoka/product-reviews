...
const CurrentThemeLayout = ({ theme, supportsAppBlocks, supportsSe }) => {
	const appBlocksUnsupportedDescription = (
		<p>
			...
			<TextStyle variation="strong">{theme.name}</TextStyle>) does not
			support app blocks.
		</p>
	);
			...
			<Card>
				<Card.Section>
					<Stack vertical>
						<GettingStartedStep
							...
							description={
								!supportsSe &&
								sectionsEverywhereUnsupportedDescription
							}
						/>
						<GettingStartedStep
							...
							description={
								!supportsAppBlocks &&
								appBlocksUnsupportedDescription
							}
						/>
					</Stack>
				</Card.Section>
				...
				<Card.Section>
					...
					{(!supportsAppBlocks || !supportsSe) && (
						<TextContainer>
							<p>
								It looks like your theme does not fully support
								the functionality of this app.
							</p>
							<p>
								Try switching to a different theme or contacting
								your theme developer to request support.
							</p>
						</TextContainer>
					)}
				</Card.Section>
...
const GettingStarted = () => {
	...
	const fetcher = useMemo(() => {
		return async (uri, options) => {
			return fetch(app)(uri, options).then((response) =>
				response?.json()
			);
		};
	}, [app]);
